...
>BTW,系统的个数可能是奇数也可能是偶数,奇数两个状态{被大多数接受,未被大多数接受},偶数三个给状态{被同样个数acceptor接受(忽略),被大多数接受,未被大多数接受}.
>被大多数接受这个要求可以避免脑裂的问题,但是如果系统个数是偶数个,而且集群系统分布在两个机房(对半分),并通过TCP互联.如果两个机房断网了,那么我们的集群显然就不能用了,因为两边都不能获得大多数的支持.所以系统数量应该是大于3的奇数个.

回到前面,状态机找到了,回顾目前我们对算法的定义,可以明显的发现一个问题,如果 P1. acceptor必须接收第一个它收到的proposal. ,那么达到系统数/2(向下取整)+1的提出的议题如何让被接受其他议题的少数派接受自己.所以这里要求acceptor必须能接受多个收到的议题.  
